...
void Get_Angle(void)
{
		...
		MPU_Gyro_Y=(MPU_I2C_ReadOneByte(devAddr,MPU6050_RA_GYRO_YOUT_H)<<8)+MPU_I2C_ReadOneByte(devAddr,MPU6050_RA_GYRO_YOUT_L);    //读取Y轴陀螺仪
		...
		if(MPU_Gyro_Y>32768)  MPU_Gyro_Y-=65536;     //数据类型转换  也可通过short强制类型转换
		if(MPU_Gyro_X>32768)  MPU_Gyro_X-=65536;
		if(MPU_Gyro_Z>32768)  MPU_Gyro_Z-=65536;
		Gyro_Y=MPU_Gyro_Y/16.4;                                    //陀螺仪量程转换	
		Gyro_X=MPU_Gyro_X/16.4;                                    //陀螺仪量程转换	
//		Gyro_Z=MPU_Gyro_Z/16.4;                                    //陀螺仪量程转换	

		MPU_Accel_Z=(MPU_I2C_ReadOneByte(devAddr,MPU6050_RA_ACCEL_ZOUT_H)<<8)+MPU_I2C_ReadOneByte(devAddr,MPU6050_RA_ACCEL_ZOUT_L);    //读取Z轴陀螺仪
		MPU_Accel_X=(MPU_I2C_ReadOneByte(devAddr,MPU6050_RA_ACCEL_XOUT_H)<<8)+MPU_I2C_ReadOneByte(devAddr,MPU6050_RA_ACCEL_XOUT_L); //读取X轴加速度记
		...
		if(MPU_Accel_Z>32768) MPU_Accel_Z-=65536;     //数据类型转换
		if(MPU_Accel_X>32768) MPU_Accel_X-=65536;    //数据类型转换
